...
class POSTagger():
    ...
    def get_unigrams(self):
        ...
        if SMOOTHING == LAPLACE:
            ...
            # For unigrams, interpolation is the same as add-k smoothing here
            m, V = 1, N
            self.unigram_probs = (self.unigram_counts + m) / (total_counts + m * V)
        elif SMOOTHING == KNESERNEY:
            # For unigrams, Kneser-Ney smoothing reduces to using relative frequencies
            self.unigram_probs = self.unigram_counts / total_counts
    ...
    def get_bigrams(self):
        ...
        self.unigram_counts = np.zeros(N)
        bigram_types = set()
        continuation_counts = np.zeros(N)

        for tags in self.data[1]:
            for i in range(len(tags)):
                idx_tag = self.tag2idx[tags[i]]
                self.unigram_counts[idx_tag] += 1
                if i > 0:
                    idx_tag_prev = self.tag2idx[tags[i - 1]]
                    self.bigram_counts[idx_tag_prev, idx_tag] += 1
                    bigram_types.add((idx_tag_prev, idx_tag))

        total_bigrams = np.sum(self.bigram_counts)
        total_unigrams = np.sum(self.unigram_counts)
        ...
        if SMOOTHING == LAPLACE:
            ...
            # Calculate number of unique continuations for each tag
            unique_continuations = np.zeros(N)
            for i in range(N):
                unique_continuations[i] = np.count_nonzero(self.bigram_counts[i, :] > 0)

            # Calculate total number of bigram types
            total_bigram_types = len(bigram_types)

            # Calculate continuation probabilities
            continuation_counts = np.zeros(N)
            for j in range(N):
                continuation_counts[j] = np.count_nonzero(self.bigram_counts[:, j] > 0)
            P_continuation = continuation_counts / total_bigram_types

            # Compute bigram probabilities with Kneser-Ney smoothing
            for i in range(N):
                for j in range(N):
                    bigram_count = self.bigram_counts[i, j]
                    unigram_count = self.unigram_counts[i]
                    unique_continuation = unique_continuations[i]
                    lambda_i = (d * unique_continuation) / unigram_count if unigram_count > 0 else 0
                    prob = max(bigram_count - d, 0) / unigram_count if unigram_count > 0 else 0
                    prob += lambda_i * P_continuation[j]
    ...
    def get_trigrams(self):
        ...
        for tags in self.data[1]:
            for i in range(len(tags)):
                ...
                if i > 0:
                    idx_tag_prev = self.tag2idx[tags[i - 1]]
                    bigram_counts[idx_tag_prev, idx_tag] += 1
                    bigram_types.add((idx_tag_prev, idx_tag))
                if i > 1:
                    idx_tag_prev_prev = self.tag2idx[tags[i - 2]]
                    self.trigram_counts[idx_tag_prev_prev, idx_tag_prev, idx_tag] += 1
                    trigram_types.add((idx_tag_prev_prev, idx_tag_prev, idx_tag))
        ...
        if SMOOTHING == LAPLACE:
            ...
            # Compute continuation counts for bigrams
            bigram_continuations = np.zeros((N, N))
            for j in range(N):
                for k in range(N):
                    bigram_continuations[j, k] = np.count_nonzero(self.trigram_counts[:, j, k] > 0)

            # Compute bigram probabilities with Kneser-Ney smoothing
            bigram_probs_kn = np.zeros((N, N))
            bigram_unique_continuations = np.zeros(N)
            for i in range(N):
                bigram_unique_continuations[i] = np.count_nonzero(bigram_counts[i, :] > 0)

            total_bigram_types = len(bigram_types)
            continuation_counts = np.zeros(N)
            for k in range(N):
                continuation_counts[k] = np.count_nonzero(bigram_counts[:, k] > 0)
            P_continuation = continuation_counts / total_bigram_types

            for i in range(N):
                for j in range(N):
                    bigram_count = bigram_counts[i, j]
                    unigram_count = self.unigram_counts[i]
                    unique_continuation = bigram_unique_continuations[i]
                    lambda_i = (d * unique_continuation) / unigram_count if unigram_count > 0 else 0
                    prob = max(bigram_count - d, 0) / unigram_count if unigram_count > 0 else 0
                    prob += lambda_i * P_continuation[j]
                    bigram_probs_kn[i, j] = prob

            # Compute trigram probabilities with Kneser-Ney smoothing
            for i in range(N):
                for j in range(N):
                    trigram_unique_continuations = np.count_nonzero(self.trigram_counts[i, j, :] > 0)
                    trigram_total = np.sum(self.trigram_counts[i, j, :])
                    lambda_ij = (d * trigram_unique_continuations) / trigram_total if trigram_total > 0 else 0
                    for k in range(N):
                        trigram_count = self.trigram_counts[i, j, k]
                        prob = max(trigram_count - d, 0) / trigram_total if trigram_total > 0 else 0
                        prob += lambda_ij * bigram_probs_kn[j, k]
                        self.trigram_probs[i, j, k] = prob
    ...
    def get_emissions(self, threshold=None):
        ...
        elif SMOOTHING == INTERPOLATION or SMOOTHING == KNESERNEY:
            # For emissions, we can use linear interpolation or relative frequencies
            if LAMBDAS is None:
                lambda_1 = 0.9
                lambda_2 = 0.1
            else:
