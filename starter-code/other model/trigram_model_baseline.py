...
class POSTagger():
    ...
    def get_emissions(self, threshold=None):
        ...
        for words, tags in zip(self.data[0], self.data[1]):
            for word, tag in zip(words, tags):
                ...
                idx_word = self.word2idx.get(word, 0)
    ...
    def sequence_probability(self, sequence, tags):
        ...
        for i, word in enumerate(sequence):
            ...
            if i == 0:
                ...
                transition_prob = self.trigram_probs[idx_pre_tag1, idx_pre_tag2, idx_tag]
        ...
        for i, word in enumerate(sequence):
            ...
            if i == 0:
                for idx in range(N_tag):
                    ...
                    total_log_prob = np.log(emission_prob) + np.log(
                        unigram_prob) if emission_prob > 0 and unigram_prob > 0 else float('-inf')
                ...
                for idx in range(N_tag):
                    ...
                    total_log_prob = np.log(emission_prob) + np.log(
                        transition_prob) if emission_prob > 0 and transition_prob > 0 else float('-inf')
                    if total_log_prob > prob_cur:
                        tag_cur = self.idx2tag[idx]
                        prob_cur = total_log_prob
            else:
                ...
                for idx in range(N_tag):
                    emission_prob = self.lexical_probs[idx, idx_word]
                    transition_prob = self.trigram_probs[prev_tag_idx1, prev_tag_idx2, idx]
                    total_log_prob = np.log(emission_prob) + np.log(
                        transition_prob) if emission_prob > 0 and transition_prob > 0 else float('-inf')
                    if total_log_prob > prob_cur:
                        tag_cur = self.idx2tag[idx]
    ...
    def beam_search(self, sequence, k):
        ...
        for i in range(N_tag):
            ...
            if emission_prob > 0 and unigram_prob > 0:
                ...
                path = [i]  # 存储标签索引
                heapq.heappush(beam_path, (-total_log_prob, path))
        ...
        for t in range(1, N_word):
            ...
            for neg_log_prob, path in beam_path:
                for j in range(N_tag):
                    ...
                    if t == 1:
                        ...
                        transition_prob = self.trigram_probs[prev_tag_idx1, prev_tag_idx2, j]
    ...
    def viterbi(self, sequence):
        ...
        for u in range(N_tag):
            emission_prob = max(self.lexical_probs[u, word_idx], min_prob)
            pi[0, 0, u] = np.log(self.unigram_probs[u]) + np.log(emission_prob)
            backpointer[0, 0, u] = 0

        if N_word > 1:
            word_idx = self.word2idx.get(sequence[1], self.word2idx.get('<UNK>', 0))
            for u in range(N_tag):
                for v in range(N_tag):
                    emission_prob = max(self.lexical_probs[v, word_idx], min_prob)
        ...
        for t in range(2, N_word):
            word_idx = self.word2idx.get(sequence[t], self.word2idx.get('<UNK>', 0))
            for u in range(N_tag):  # t-1 时刻的标签
                for v in range(N_tag):  # t 时刻的标签
                    emission_prob = max(self.lexical_probs[v, word_idx], min_prob)
                    max_prob = float('-inf')
                    best_w = 0
                    for w in range(N_tag):  # t-2 时刻的标签
        ...
        tags = [self.idx2tag[idx] for idx in tags_idx]
        return tags
